<nav class="sidebar h-full flex flex-col justify-between">
    <div>
        ...
        <div class="mt-4 space-y-2">
            ...
            <x-nav-link-custom :href="route('admin.empresas.index')" :active="request()->routeIs('admin.empresas.*')">
                <x-slot name="icon">
                    <svg class="h-6 w-6" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M2.25 21h19.5m-18-18v18m10.5-18v18m6-13.5V21M6.75 6.75h.75m-.75 3h.75m-.75 3h.75m3-6h.75m-.75 3h.75m-.75 3h.75M6.75 21v-3.375c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125V21M3 3h12m6.75 4.5l-1.5-1.5m0 0l-1.5 1.5m1.5-1.5V21" /></svg>
                </x-slot>
                Gerenciar Empresas
            </x-nav-link-custom>
            @endif

            {{-- Links do Usuário da Empresa --}}
            @if(auth()->user()->role !== 'super-admin')
    ...
    <div class="p-4 border-t border-gray-700">
        <form method="POST" action="{{ route('logout') }}">
            @csrf
            <x-nav-link-custom :href="route('logout')"
                    :active="false"
