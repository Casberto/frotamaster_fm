@if(Auth::user()->temPermissao('SEG001') || Auth::user()->temPermissao('SEG002'))
<div x-data="{ open: false }" class="menu-item-group">
    <button @click="open = !open" class="sidebar-link w-full flex justify-between items-center">
        <div class="flex items-center">
            ...
            <span class="menu-item-text">Seguros</span>
        </div>
    ...
    <div x-show="open" x-transition class="pl-8 space-y-1 mt-1 submenu-container">
        ...
        <a href="{{ route('seguros.index') }}" class="sidebar-submenu-link @if(request()->routeIs('seguros.index')) active @endif">
            <span class="mr-3">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-5">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M8.25 6.75h12M8.25 12h12m-12 5.25h12M3.75 6.75h.007v.008H3.75V6.75Zm.375 0a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0ZM3.75 12h.007v.008H3.75V12Zm.375 0a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Zm-.375 5.25h.007v.008H3.75v-.008Zm.375 0a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Z" />
                </svg>
            </span>
            <span class="submenu-item-text">Listar Apólices</span>
        </a>
        @endif

        @if(Auth::user()->temPermissao('SEG002'))
        <a href="{{ route('seguros.create') }}" class="sidebar-submenu-link @if(request()->routeIs('seguros.create')) active @endif">
            <span class="mr-3">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-5">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M12 4.5v15m7.5-7.5h-15" />
                </svg>
            </span>
            <span class="submenu-item-text">Nova Apólice</span>
        </a>
        @endif
    </div>
</div>
