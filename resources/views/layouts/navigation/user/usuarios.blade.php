@if(Auth::user()->hasPermission('Usuários', 'visualizar') || Auth::user()->hasPermission('Usuários', 'criar') || Auth::user()->hasPermission('Perfis', 'visualizar'))
<div x-data="{ open: false }" class="menu-item-group">
    ...
    <div x-show="open" x-transition class="pl-8 space-y-1 mt-1 submenu-container">
        @if(Auth::user()->hasPermission('Usuários', 'visualizar'))
        <a href="{{ route('usuarios.index') }}" class="sidebar-submenu-link {{ request()->routeIs('usuarios.index') ? 'active' : '' }}">
            ...
            <span class="submenu-item-text">Listar Usuários</span>
        </a>
        @endif
        
        @if(Auth::user()->hasPermission('Usuários', 'criar'))
        <a href="{{ route('usuarios.create') }}" class="sidebar-submenu-link {{ request()->routeIs('usuarios.create') ? 'active' : '' }}">
            ...
            <span class="submenu-item-text">Novo Usuário</span>
        </a>
        @endif
        ...
        <a href="{{ route('perfis.index') }}" class="sidebar-submenu-link @if(request()->routeIs('perfis.*')) active @endif">
            <span class="mr-3"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-5"><path stroke-linecap="round" stroke-linejoin="round" d="M17.982 18.725A7.488 7.488 0 0012 15.75a7.488 7.488 0 00-5.982 2.975m11.963 0a9 9 0 10-11.963 0m11.963 0A8.966 8.966 0 0112 21a8.966 8.966 0 01-5.982-2.275M15 9.75a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z" /></svg></span>
            <span class="submenu-item-text">Perfis</span>
        </a>
        @endif
    </div>
</div>
