@if(
    Auth::user()->hasPermission('Motoristas', 'visualizar') || Auth::user()->hasPermission('Motoristas', 'criar') ||
...
<div x-data="{ open: false }">
    ...
    <div x-show="open" x-transition class="pl-8 space-y-1 mt-1">
        ...
        {{-- Motoristas --}}
        @if(Auth::user()->hasPermission('Motoristas', 'visualizar'))
        <a href="{{ route('motoristas.index') }}" class="sidebar-submenu-link @if(request()->routeIs('motoristas.index')) active @endif">
            ...
            <span>Listar Motoristas</span>
        </a>
        @endif
        @if(Auth::user()->hasPermission('Motoristas', 'criar'))
